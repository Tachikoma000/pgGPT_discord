...
async def handle_ai_interaction(int, question):
    try:
        ...
        if not response or not isinstance(response, list) or len(response) < 1 or not isinstance(response[0], (list, tuple)) or len(response[0]) < 2:
        ...
        # Splitting the response into multiple fields if it's too long
        char_limit = 1024
        for idx, part in enumerate(response_parts):
            while len(part) > char_limit:
                # Split the part into a chunk that fits within the limit and the remaining content
                chunk, part = part[:char_limit], part[char_limit:]
                title = f"**Response {idx + 1} (Cont'd):**"
